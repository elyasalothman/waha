...
import { chromeNav, isFirstRowPlayPath, mobileChromeNav, PERSONAL_NAV, TOOLS_OVERFLOW_NAV, WORK_NAV } from "./nav.ts";
...
describe("chrome nav lock", () => {
  it("keeps Play off the first-row personal and work bars", () => {
    ...
    assert.equal(PERSONAL_NAV.some((item) => item.key === "games"), false);
    assert.equal(WORK_NAV.some((item) => item.key === "games"), false);
    assert.equal(isFirstRowPlayPath("/games"), false);
  });
