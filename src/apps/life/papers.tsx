...
const KINDS = [
  { id: "id", ar: "هوية / إقامة", en: "ID / Iqama" },
  { id: "passport", ar: "جواز", en: "Passport" },
  { id: "license", ar: "رخصة قيادة", en: "License" },
  { id: "health", ar: "تأمين طبي", en: "Health cover" },
  { id: "reg", ar: "استمارة", en: "Registration" },
...
export function PapersApp() {
  const lang = useAppStore((s) => s.lang);
  const [docs, setDocs] = usePersistent<Doc[]>("waha:papers", []);
  ...
  return (
    <div className="space-y-4">
      <form
        className="grid gap-2 sm:grid-cols-[1fr_8rem_9rem_auto]"
        onSubmit={(e) => {
          e.preventDefault();
      ...
      >
        <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={L("اسم الوثيقة", "Document name")} />
      ...
      {sorted.length === 0 ? (
        <p className="text-sm text-muted">{L("أضف هويتك وجوازك ورخصتك ليصلك تنبيه قبل انتهائها.", "Add your ID, passport, and license to get a reminder before they lapse.")}</p>
      ) : (
          ...
          {sorted.map((doc) => {
            ...
            const label =
              d == null
                ? ""
                : d < 0
                  ? L(`انتهت منذ ${Math.abs(d)} يوماً`, `Expired ${Math.abs(d)} days ago`)
                  : d === 0
                    ? L("تنتهي اليوم", "Expires today")
                    : L(`متبقي ${d} يوماً`, `${d} days left`);
            return (
