...
test("does not inject grok-app-builder extensions.js without a project id", () => {
...
test("injects project id meta without grok-app-builder script", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", {
    appName: "Demo",
    projectId: "proj-123",
  });
...
test("strips grok-app-builder extensions.js from existing html", () => {
  const ctx = { appName: "Demo", projectId: "proj-123" };
  const dirty =
    '<html><head><script src="https://grok.com/grok-app-builder/extensions.js" data-project-id="proj-123" defer></script></head></html>';
  const once = injectGrokPwaHead(dirty, ctx);
  const twice = injectGrokPwaHead(once, ctx);
  assert.equal(once, twice);
  assert.doesNotMatch(once, /grok-app-builder\/extensions\.js/);
